...
pub trait TranslationRegime: private::Sealed + Copy + Clone + Debug + Eq + PartialEq + 'static + Send + Sync {
    type Attributes: PagingAttributes;

    fn supports_asid() -> bool;
...
impl private::Sealed for El1And0 {}
...
impl private::Sealed for El2And0 {}
...
pub trait Translation<A: PagingAttributes = El1Attributes> {
...
impl<R: TranslationRegime, T: Translation<R::Attributes>> RootTable<R, T> {
    ...
    pub fn new(mut translation: T, level: usize, _regime: R, va_range: VaRange) -> Self {
        ...
        if !R::supports_asid() && va_range != VaRange::Lower {
...
#[derive(Debug)]
pub(crate) struct PageTableWithLevel<T: Translation<A>, A: PagingAttributes = El1Attributes> {
...
#[repr(C, align(4096))]
pub struct PageTable<A: PagingAttributes = El1Attributes> {
...
mod tests {
    ...
    fn debug_roottable_empty() {
        let table = RootTable::<El1And0, TargetAllocator>::new(
...
0  -511: 0
    ...
    fn debug_roottable_contiguous() {
        let mut table = RootTable::<El1And0, TargetAllocator>::new(
            TargetAllocator::new(0),
            1,
            El1And0,
            VaRange::Lower,
