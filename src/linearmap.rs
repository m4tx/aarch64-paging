...
use crate::{
    MapError, Mapping,
    descriptor::{
        Descriptor, El1Attributes, PagingAttributes, PhysicalAddress, UpdatableDescriptor,
        VirtualAddress,
    },
    paging::{
...
pub struct LinearTranslation<A: PagingAttributes = El1Attributes> {
...
impl<R: TranslationRegime> LinearMap<R> {
    ...
    ///
    /// The `offset` must be a multiple of [`PAGE_SIZE`]; if not this will panic.
    pub fn new(asid: usize, rootlevel: usize, offset: isize, regime: R, va_range: VaRange) -> Self {
        Self {
            mapping: Mapping::new(
